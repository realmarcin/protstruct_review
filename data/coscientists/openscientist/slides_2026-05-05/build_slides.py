#!/usr/bin/env python3
"""
Generate landscape SVG slides summarising the 2026-05-05 1SAR refinement re-run.
...
def slide_01_title():
    ...
    body.append(
        ...
        f'Mostly pass · 2026-05-05 re-run · cross-tool coverage strengthened on T03</text>'
    )
    body.append(
        f'  <text x="80" y="420" font-size="22" fill="{MUTED}">'
        f'11 independent oracle code paths · 9 non-cctbx · all 2026-05-04 findings reproduce · Servalcat sigmaa adds 4th R-work/R-free pair</text>'
    )

    # Bullet points (key new since prior round)
    bullets = [
        ("R-free reproducibly ≈ 0.21, not 0.199 — 4 code paths now (PHENIX 0.211 · gemmi 0.217 · REFMAC 0.213 · Servalcat 0.221)", BAD),
        ("Round 6 R-free 0.207 better than round 7 R-free 0.212 — claim reversed (re-measured)", BAD),
        ...
        ("T13 data quality clean — no twinning · no tNCS · ΔB aniso 7.3 Å²", GOOD),
    ]
    yb = 490
...
def slide_02_oracles():
    body = [header(
        "Oracle stack — 11 independent code paths",
        "9 non-cctbx · trust comes from cross-family agreement, not from any single tool · re-run 2026-05-05"
    )]
    ...
    body.append(
        ...
        f'non-cctbx (10 binaries across 9 code paths)</text>'
    )
    nons = [
        ...
        ("Servalcat sigmaa (NEW R-work/R-free)", "Murshudov — direct R-work/R-free + σ_A"),
        ...
        ("ctruncate", "CCP4 — Wilson B · L-test twinning · ΔB aniso · tNCS · ice rings"),
    ]
    ...
    rows = [
        ...
        ["T03 R-factors", "phenix.model_vs_data", "gemmi · Servalcat sigmaa · REFMAC5", "strengthened"],
...
def slide_03_verdict():
    body = [header(
        "Headline verdict — 2026-05-05 re-run",
        "Mostly pass · 3 triangulated discrepancies · K⁺ alternative still open · cross-tool coverage strengthened on T03"
    )]
    ...
    body.append(
        ...
        f'Closed across the 04-30 / 05-01 / 05-04 / 05-05 re-issues</text>'
    )
    closed = [
        "Mg²⁺ alternative ruled out — CheckMyMetal-style |Z| 16.35",
        "Ligand / metal density support confirmed — RSCC ≥ 0.96",
        "T13 data-quality cross-tool gap closed — ctruncate (05-04)",
        "T03 strengthened — Servalcat sigmaa direct R-work/R-free, 4-tool bracket (NEW 05-05)",
    ]
    ...
    still = [
        "R-free reproducibly ≈ 0.21 (4-tool bracket), agent reports 0.199",
        "Round 6 R-free 0.207 < round 7 R-free 0.212",
    ...
    summary = [
        ...
        ("R-free gap < 0.05", "⚠ 3 of 4 oracles fail", BAD),
        ("Ramachandran < 1%", "✓ headline wrong", WARN),
        ("Clashscore < 5", "✓ pass", GOOD),
        ("No Δρ > 4σ", "✗ fails", BAD),
        ("Data quality (T13)", "✓ pass", GOOD),
    ]
    cx = 80
    cw = (W - 160 - 5 * 20) // 6
...
def slide_04_checklist():
    body = [header(
        "Skill checklist — protstruct-eval applied end-to-end (2026-05-05 re-run)",
        "All 8 steps green · plus water / ligand / metal sub-checklist · every value re-measured today"
    )]
    ...
    rows = [
        ...
        ["6", "Per-round Δ-claim", "round 6 R-free 0.2068 < round 7 0.2116 (re-measured)"],
        ["7", "Non-cctbx confirmation per finding",
         "9 / 11 oracles tagged · cross_tool_coverage all closed"],
        ["8", "T13 data-quality oracle",
...
def slide_06_t03_rfactors():
    body = [header(
        "T03 — Reciprocal-space R-factors · 4 tools converge (Servalcat sigmaa added 2026-05-05)",
        "Agent’s 0.199 R-free is 0.012 below the lower bound · 3 of 4 oracles fail R-free gap < 0.05"
    )]
    ...
    rows = [
        ...
        ["Servalcat sigmaa (NEW)", "0.171", "0.221", "0.050"],
    ...
    for r in rows:
        ...
        elif r[0].startswith("Servalcat"):
            fills.append([GOOD_LIGHT, None, None, None])
            txt.append([None]*4)
        else:
            fills.append([None]*4)
            txt.append([None]*4)
    body.append(
        table(80, 200, [430, 200, 200, 200], headers, rows,
              row_height=52, font_size=22, header_font_size=22,
              cell_fills=fills, cell_text_colors=txt,
    ...
    lo, hi = 0.195, 0.225
    for v in [0.195, 0.200, 0.205, 0.210, 0.215, 0.220, 0.225]:
    ...
    body.append(
        ...
        f'Four independent code paths converge on R-free 0.211–0.221.</text>'
    )
    body.append(
    ...
    body.append(
        ...
        f'PHENIX gap 0.055 (✗) · gemmi 0.053 (✗) · Servalcat 0.050 (✗) · REFMAC5 0.044 (✓)</text>'
    )
    body.append(
        f'  <text x="1000" y="660" font-size="18" fill="{INK}">'
        f'Catalog oracle of record = phenix.model_vs_data → fails &lt; 0.05 (3 of 4).</text>'
    )
    body.append(
...
def slide_07_per_round():
    body = [header(
        "T03 — Per-round trajectory · oracle re-measurement (2026-05-05)",
        "Round 6 R-free 0.207 beats round 7 R-free 0.212 · NCS-torsion claim doesn’t reproduce at oracle scaling"
    )]
    ...
    body.append(
        ...
        f'R-free <tspan font-weight="700">0.207 → 0.212</tspan></text>'
    )
    body.append(
        f'  <text x="{sx+20}" y="{chart_y+115}" font-size="18" fill="{INK}">'
        f'Δ = +0.005 (worse)</text>'
    )
    body.append(
...
def slide_08_t05_geometry():
    ...
    rows = [
        ...
        ["Clashscore (non-cctbx)", "MolProbity standalone", "3.82 (H-build variance)", "✓ < 5"],
...
def slide_13_t13():
    body = [header(
        "T13 — X-ray data-quality oracle (re-measured 2026-05-05, identical to 2026-05-04)",
        "scripts/t13_data_quality.py · CCP4 ctruncate (with aimless attempted) · cross-tool gap remains closed"
    )]
    ...
    body.append(
        ...
        f'T13 was wired on 2026-05-04; values are bit-identical on 2026-05-05 re-run.</text>'
    )
    body.append(
...
def slide_14_assumptions():
    body = [header(
        ...
        "QDS assumptions_report 36+ rows · 2026-05-05 re-run hardens R-free finding from 3-tool to 4-tool"
    )]
    ...
    rows = [
        ...
        ["ASSUM_phenix_refine_in_run_r_factors_use_internal_scaling",
         "known_violation", "known_violation (4-tool hardened, NEW 05-05)"],
        ...
        ["ASSUM_t13_oracle_pending", "open follow-up", "closed — ctruncate wired"],
        ["ASSUM_aimless_requires_unmerged_intensities",
         "—", "mitigated — wrapper falls back to ctruncate"],
        ["ASSUM_servalcat_sigmaa_per_shell_only  (NEW 05-05)",
         "mitigated", "still mitigated · sigmaa direct R-work/R-free now also captured"],
    ]
    fills = []
    txt = []
    for r in rows:
    ...
    body.append(
        table(80, 190, [800, 360, 600], headers, rows,
              row_height=42, font_size=16, header_font_size=20,
              cell_fills=fills, cell_text_colors=txt,
...
def slide_15_net():
    ...
    issues = [
        ("R-free under-stated", "≈ 0.012 below 4-tool consensus", BAD,
         "Four code paths bracket R-free 0.211–0.221. Success-criterion gap < 0.05 fails 3 of 4."),
        ("Round 7 ≮ Round 6", "0.212 vs 0.207 at oracle scaling", BAD,
    ...
    body.append(
        ...
        f'Strengthened in this re-run (2026-05-05)</text>'
    )
    body.append(
        f'  <text x="{80 + (W-160-30)//2 + 50}" y="755" font-size="18" fill="{INK}">'
        f'Servalcat sigmaa direct R-work/R-free → 4-tool R-free bracket</text>'
    )
    body.append(
        f'  <text x="{80 + (W-160-30)//2 + 50}" y="790" font-size="17" fill="{INK}">'
        f'PHENIX 0.211 · gemmi 0.217 · REFMAC 0.213 · Servalcat 0.221 — agent 0.199 below all four.</text>'
    )
    body.append(
        f'  <text x="{80 + (W-160-30)//2 + 50}" y="822" font-size="17" fill="{INK}">'
        f'Every 2026-05-04 finding reproduces deterministically against the same artefact.</text>'
    )
    body.append(
        f'  <text x="{80 + (W-160-30)//2 + 50}" y="848" font-size="17" fill="{INK}">'
        f'Cross-tool coverage matrix fully green; T13 + T03 both have ≥ 4 oracles.</text>'
    )
    return "\n".join(body) + "\n"
