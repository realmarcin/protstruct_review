...
Applies the protstruct_review evaluation framework (catalog tasks T01, T03, T05, T06) to the artifact at `cdba2c07-daff-4f60-ae96-12452b3a5fbb_artifacts.zip`. The agent ran 7 rounds of `phenix.refine` on PDB `1SAR` (ribonuclease Sa, 2.50 Å, P 2₁ 2₁ 2₁) starting from the tutorial-derived `1sar.pdb` + `1sar.mtz` inputs in that artifact and produced `1sar_final.pdb` + `1sar_final.mtz`.
...
- `EVAL_1sar_cdba2c07_2026-04-24.md` — this report
- `EVAL_1sar_cdba2c07_2026-04-24_metrics.tsv` — granular: one row per (task, metric, oracle, stage); machine-loadable
- `EVAL_1sar_cdba2c07_2026-04-24_headline.tsv` — collapsed: one row per top-level finding (chat-summary / leaderboard view)
- `gemmi_rfactor.py` — non-cctbx R-factor cross-check script (re-used across 1SAR evals; not date-stamped)

Naming convention: `EVAL_<structure>_<artifact-short-id>_<YYYY-MM-DD>.{md,tsv}` with parallel `_<view>` TSV suffixes — see `ref/eval_naming.md`.
