...
| LinkML schema, the catalog, and the reference tables | CC-BY-4.0 (this file; the schema declares it in its header) | `schemas/`, `ref/catalog.yaml`, `ref/tasks_and_evaluations.tsv`, `ref/structural_criteria.yaml`, `ref/tool_assumptions.yaml`, `ref/tool_recommendations.yaml` |
...
| Derived structures and bundled agent artefacts: refined coordinate/reflection files produced from deposited entries, third-party agent outputs | **Upstream terms of the source entry or producer control — not relicensed here** | `data/agents/*/**/*.pdb`, `*.mtz`; `data/coscientists/**/*.pdf`, `*.zip`; see [`data/README.md`](data/README.md) |
