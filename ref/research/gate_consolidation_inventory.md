...
|---|---|---|---|
| Tolerance-series round count — `NEXT_TASKS.md` ("N rounds of …", spelled out) | the set of `tolerance_benchmark_round*.md` | `NEXT_TASKS.md` | `check_summary_coverage.round_count_claim`: renders the expected phrase with `spell()` (fails closed outside 20–99 since PR #159, issue #160) and locates it with a `_TENS`-derived alternation (went MISSING at round 40 until derived, round-40 PR #265; #244 was the same MISSING-on-correct-prose class in `check_round_figures --refresh`, and the comment at `check_summary_coverage.py:262` still credits it — #468) |
