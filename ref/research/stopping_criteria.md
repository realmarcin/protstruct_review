# Stopping and consolidation criteria for the round cadence (#292)

The stopping discipline that has actually governed rounds lives as scattered
...
Apply before registering a round, and again before extending a line of
investigation into another round.

1. **Power first.** Is the achievable sample large enough to detect the
   effect the question asks about? If not, widen the population or drop the
   question — do not re-verify the same lines harder. *(Rounds 31–33: the
   verification inventory yields ~58–101 claims where the test needs 130+;
   the question was closed as unanswerable there, and that closure was the
   round's result.)*

2. **Population vs question.** A result that exhausts the available set
   answers about the set, not about the question the set was chosen to
...
4. **An underpowered null is not a refutation.** `0 of 13` above a fence is
   ...
   alongside the count, and let the power decide whether the line continues.
   *(Rounds 23 and 34; round 34 also showed the batch's ceiling is usually
...
6. **Pre-registered stop rule.** Every round states its stop/continue
   ...
   registered (the mark stays; the falsification is reported); a better
   measure or instrument is its own pre-registered change, never a silent
   switch mid-round. *(Rounds 44–48 in the tolerance series; the
   negative-control track's D6 stop, U3/H1 re-registration, and the
   round-11 amendment that refused itself by its own safety clause are the
   same rule enforced.)*

## Consolidation triggers

A line of work is consolidated — closed with its findings summarized, its
open questions either filed or explicitly dropped — when any of these
holds:

- the registered question is **answered** (either direction), and successor
  questions are new registrations, not extensions;
...
Not a tolerance, not a gate, and not a license to skip a registered stop —
