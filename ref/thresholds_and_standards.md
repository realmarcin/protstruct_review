...
| Secondary-structure agreement | agent-vs-DSSP three-state ≥ **0.85** over DSSP-assigned residues *(unmeasured — a different comparison)*; two independent assigners floor ≥ **0.65**, **not 0.80**. Benchmarked on 16 well-ordered structures: median **0.753**, range 0.679–0.850, and the 0.80 floor **failed on 12/16** including ubiquitin, lysozyme and trypsin. It had been set from 1SAR (0.865), which scores higher than every entry in the benchmark. Agreement is fold-class dependent — expect **α-rich ~0.80–0.85, β-rich ~0.68–0.72** — because P-SEA infers strands from Cα geometry and DSSP from backbone H-bonds | agent vs DSSP; DSSP vs biotite P-SEA (`t15_ss_agreement.py`) | `[benchmark]` (`ref/research/tolerance_benchmark_ss_agreement.md`) |
