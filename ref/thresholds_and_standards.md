...
| Geometry did not degrade (**Δ clause**) | clashscore_post ≤ clashscore_pre + **4**; favored_post ≥ favored_pre − **1.5 pp**; rotamer outliers_post ≤ outliers_pre + **4 pp**. **Clashscore is a poor degradation signal for X-ray refinement** — it *worsened* in 6 of 8 null re-refinements (median **+1.87**, max +10.39). Treat a clashscore rise alone as informational; require a second signal (favored % or rotamer outliers) before calling a refinement degrading | `[benchmark]` (`ref/research/tolerance_benchmark_refinement_deltas.md`) |
