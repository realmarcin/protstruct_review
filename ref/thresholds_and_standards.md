...
| Map-model fit did not degrade | CC_mask_post ≥ CC_mask_pre − **0.01** (retained, but a null real-space refinement consumed **65 %** of the band on 1 of 2 EM entries — revisit with a larger set). **`d_FSC_model` is not gateable** — and **half-maps do not fix it** (tested). Supplying both half-maps makes the *half-map* FSC work (27WR: d_fsc 2.61 Å) while leaving the *model-map* crossings degenerate (FSC = 0.5 still at 29.79 Å). **The cause is not identified.** A coverage explanation was proposed from 2 entries and **withdrawn** when 4 more were added: 21BQ sits at 71 atoms per 10⁶ Å³ of map box — within 2.6× of the failing 9VJD's 27 — and produces a perfectly sensible value. Failure rate is **1 of 6** EM entries; low coverage is not sufficient to cause it and may not be necessary | `[benchmark]` (`ref/research/tolerance_benchmark_refinement_deltas.md`) |
