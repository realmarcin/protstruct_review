...
| Ramachandran / rotamer favored % | **Ramachandran: ± 0.2 pp** — measured against the wwPDB validation report over 17 entries (median **0.00 pp**, p90 0.02, max **0.16 pp**), so ± 1.0 pp was ~6× too loose. Unlike the *outlier* row this evidence is informative on all 17 entries, since favored % is never degenerate. **Rotamer favored %: still unmeasured** — the report's `rota=` attribute is the rotamer *name* (`m-10`, `mp`, …), not a favored/allowed verdict, so the wwPDB reference does not exist; it needs a different oracle | `phenix.ramalyze` vs wwPDB validation report (per-residue `rama=` counts) | `[benchmark]` (`ref/research/tolerance_benchmark_vs_deposited.md`) |
...
| Geometry did not degrade (**Δ clause**) | favored_post ≥ favored_pre − **1.5 pp** (worst null-case drop 1.41 pp); rotamer outliers_post ≤ outliers_pre + **4 pp** (worst null-case rise 3.60 pp). **No clashscore Δ gate**: on a null re-refinement clashscore moved in *both* directions (4 up, 4 down, signed median +0.78) with a range of −2.70 to **+10.39**, so any band covering it would permit a clashscore to quadruple and any tighter band is breached by a refinement that changed nothing. Report the clashscore Δ as **informational**, and never call a refinement degrading on a clashscore rise alone — require favored % or rotamer outliers to agree | `[benchmark]` (`ref/research/tolerance_benchmark_refinement_deltas.md`) |
