...
|---|---|---|
| ΔRMSD sanity | **Resolution-conditional.** `d_min < 2.5 Å`: RMSD_post ≤ RMSD_pre + **0.10 Å** (null max 0.075 Å). `d_min ≥ 2.5 Å`: + **0.35 Å** (null max 0.285 Å). Validated on **32 entries, 1.37–3.60 Å** (14 below 2.5 Å, of which 9 below 2.0 Å): **0/32** violations. But the `< 2.5 Å` band has only **1.15× headroom** (largest null shift 0.0867 Å against 0.10 Å) — round 8's reported 2× was an artefact of having just 3 high-resolution entries. Treat it as at the edge of its evidence and re-check when entries are added. The underlying dependence is smooth (r = 0.59 with d_min), but a resolution-scaled band is no better for Cα shift and *worse* for favored drop, so the step is kept. A null re-refinement moves a low-resolution model **4× further** than a high-resolution one, so a single band cannot serve both — the earlier +0.05 Å flagged 5/8 correct refinements, and its +0.15 Å replacement (fitted to a 1.37–2.92 Å set) was itself breached by 4/19 once low-resolution entries were included | `[benchmark]` (`ref/research/tolerance_benchmark_refinement_deltas.md`) |
