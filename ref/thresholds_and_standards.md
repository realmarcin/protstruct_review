...
| Geometry did not degrade (**Δ clause**) | favored_post ≥ favored_pre − **1.5 pp** (worst null-case drop 1.41 pp); rotamer outliers_post ≤ outliers_pre + **4 pp** (worst null-case rise 3.60 pp). **Clashscore: gate on the ratio, not the difference** — `clashscore_post / clashscore_pre ≥ 5×` is evidence of degradation (null re-refinement ≤ **4.26×**; a 0.1 Å coordinate perturbation gives ≥ **8.5×**). The *difference* stays ungated: on a null re-refinement it moved in both directions (4 up, 4 down, signed median +0.78, range −2.70 to +10.39), so no difference band works. Clashscore is nonetheless the **most sensitive** §4 signal — at 0.05 Å damage it is the only quantity that moves at all | `[benchmark]` (`ref/research/tolerance_benchmark_refinement_deltas.md`) |
