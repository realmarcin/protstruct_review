...
| Ramachandran / rotamer favored % | **Ramachandran: ± 0.2 pp** — measured against the wwPDB validation report over 17 entries (median **0.00 pp**, p90 0.02, max **0.16 pp**), so ± 1.0 pp was ~6× too loose. Unlike the *outlier* row this evidence is informative on all 17 entries, since favored % is never degenerate. **Rotamer favored %: ± 1.0 pp retained, not directly measurable** — the report's `rota=` attribute is the rotamer *name* (`m-10`, `mp`, …), not a favored/allowed verdict, so no reference figure exists. The underlying **assignment** is reproduced exactly, however: **8054 of 8054 residues** across 17 entries get the identical rotamer name from `phenix.rotalyze` and the wwPDB report, so assignment disagreement is not a risk to the band; only the favored/allowed boundary is unverified | `phenix.ramalyze` vs wwPDB validation report (per-residue `rama=` counts) | `[benchmark]` (`ref/research/tolerance_benchmark_vs_deposited.md`) |
...
> detectable by any §4 clause** — 7 of 9 models damaged at σ = 0.05 Å (≈ 0.09 Å shift) passed
...
|---|---|---|
| ΔRMSD sanity | RMSD_post ≤ RMSD_pre + **0.15 Å**. A null re-refinement moves Cα by a median **0.072 Å**, max **0.107 Å**, so the retired + 0.05 Å was tighter than the refinement program's own reproducibility and flagged **5/8** correct refinements as degradation | `[benchmark]` (`ref/research/tolerance_benchmark_refinement_deltas.md`) |
| Geometry did not degrade (**Δ clause**) | favored_post ≥ favored_pre − **1.5 pp** (worst null-case drop 1.41 pp); rotamer outliers_post ≤ outliers_pre + **4 pp** (worst null-case rise 3.60 pp). **Clashscore: gate on the ratio, not the difference, while `clashscore_pre ≲ 20`** — `clashscore_post / clashscore_pre ≥ 5×` is evidence of degradation; every benchmark model separates its null ratio from its 0.1 Å-damage ratio by ≥ **6.7×**. **Above pre ≈ 20 the gate fails**: damage drives clashscore to ~100 regardless of starting value, so the ratio collapses (100/pre) — compare the **absolute** post-clashscore against §2's bar there instead. The *difference* stays ungated: on a null re-refinement it moved in both directions (4 up, 4 down, signed median +0.78, range −2.70 to +10.39), so no difference band works. Clashscore is nonetheless the **most sensitive** §4 signal — at 0.05 Å damage it is the only quantity that moves at all | `[benchmark]` (`ref/research/tolerance_benchmark_refinement_deltas.md`) |
...
| Map-model fit did not degrade | CC_mask_post ≥ CC_mask_pre − **0.01** (retained, but a null real-space refinement consumed **65 %** of the band on 1 of 2 EM entries — revisit with a larger set). **`d_FSC_model` is not gateable**: `phenix.mtriage`'s model-map FSC crossings are degenerate without half-maps (27WR reports FSC = 0.5 at 29.79 Å for a 2.7 Å map), and passing `resolution=` does not fix it | `[benchmark]` (`ref/research/tolerance_benchmark_refinement_deltas.md`) |
