...
| 46 | (2026-08-07) | **#284 closed** — settled the vs-deposited band question round 45 opened. The raw favored-/rotamer-% bands are denominator-sensitive under altloc/completeness, so the **load-bearing check is now per-shared-residue classification agreement** (do the pipelines assign the same verdict to residues they both evaluate?), robust to the denominator difference; raw-% `\|Δ\|` demoted to reported diagnostics. Rotamer verdict agreement **1.0000 on all 41**; Ramachandran (new `ramachandran_agreement`, tested) **1.0000 on 40/41, 0.9976 on 15C8** (one boundary residue, (H,100,GLY) Favored↔Allowed, named). **#4 outlier row resolves** — 16→**17 backed**, 4→**3 marked** |
